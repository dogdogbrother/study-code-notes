...
然而我们平时不会注意这么多细节,我们使用的基本都是传一个普通对象给服务端，例如：
```typescript
axios({
  method: 'post',
  url: '/base/post',
...
```
这个时候 `data`对象是不能直接传给 `send` 函数的，我们需要把它转换成 JSON 字符串。
...
// 注意 isPlainObject 不是上个章节写的函数(上个章节写的叫做isObject)
...
工具写好了,我们就在`index.ts`中使用它：
```typescript
import { transformRequest } from './helpers/data'

function processConfig (config: AxiosRequestConfig): void {
  config.url = transformURL(config)
  config.data = transformRequestData(config) // 新写的
}
